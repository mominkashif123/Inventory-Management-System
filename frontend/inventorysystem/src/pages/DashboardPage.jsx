  ...
  if (loading) return (
    <div className="min-h-screen bg-white dark:bg-gray-900 flex items-center justify-center">
      <div className="flex flex-col items-center space-y-4">
        ...
        <p className="text-orange-600 dark:text-orange-400 font-semibold text-lg">Loading dashboard...</p>
      </div>
    </div>
  );
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-50 to-white dark:from-gray-900 dark:to-gray-800">
      <div className="pt-20 pb-8 px-4 sm:px-6 lg:px-8">
        ...
        {lowStockCritical.length > 0 && (
            <div className="mb-8 bg-red-50 dark:bg-red-900/20 border-l-4 border-red-500 rounded-lg shadow-sm">
              <div className="p-6">
                <div className="flex items-center mb-4">
                  ...
                  <h3 className="ml-3 text-lg font-bold text-red-800 dark:text-red-200">Critical Stock Alert</h3>
                </div>
              ...
              {lowStockCritical.map(p => (
                ...
                <p className="text-sm text-red-700 dark:text-red-300">Click a product to update inventory levels.</p>
              </div>
          </div>
        )}
            ...
            <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-6">
              <div className="flex items-center space-x-4">
                ...
                <div>
                  ...
                  <p className="text-gray-600 dark:text-gray-400 mt-1">Real-time business analytics</p>
                </div>
                </div>
              
              {/* Key Metrics Cards */}
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 lg:gap-6">
                <div className="bg-white dark:bg-gray-800 rounded-xl shadow-lg border border-gray-100 dark:border-gray-700 p-6 text-center transform hover:scale-105 transition-all duration-200">
                  <div className="text-sm font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wide mb-2">Total Sales</div>
                  <div className="text-3xl font-bold text-orange-600 dark:text-orange-400">{summary?.total_sales || 0}</div>
                  <div className="text-xs text-green-600 dark:text-green-400 mt-2">+12% from last month</div>
                </div>
                
                <div className="bg-white dark:bg-gray-800 rounded-xl shadow-lg border border-gray-100 dark:border-gray-700 p-6 text-center transform hover:scale-105 transition-all duration-200">
                  <div className="text-sm font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wide mb-2">Revenue</div>
                  <div className="text-3xl font-bold text-orange-600 dark:text-orange-400">${Number(summary?.total_revenue || 0).toLocaleString()}</div>
                  <div className="text-xs text-green-600 dark:text-green-400 mt-2">+{growthPercentage}% growth</div>
                </div>
                
                <div className="bg-white dark:bg-gray-800 rounded-xl shadow-lg border border-gray-100 dark:border-gray-700 p-6 text-center transform hover:scale-105 transition-all duration-200">
                  <div className="text-sm font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wide mb-2">Inventory Value</div>
                  <div className="text-3xl font-bold text-orange-600 dark:text-orange-400">${Number(inventoryValue?.inventory_value || 0).toLocaleString()}</div>
                  <div className="text-xs text-blue-600 dark:text-blue-400 mt-2">Current total</div>
                </div>
                </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 mb-8 items-stretch">
            ...
            {/* Best Sellers Chart */}
            <div className="lg:col-span-4 bg-white dark:bg-gray-800 rounded-xl shadow-lg border border-gray-100 dark:border-gray-700 p-6 flex flex-col h-full">
                ...
                <>
                  ...
                  <div className="space-y-2">
                    {productBreakdown.slice(0, 5).map((p, idx) => (
                      <div key={p.name + idx} className="flex items-center justify-between p-2 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors">
                        ...
                        <span className="text-sm font-bold text-orange-600 dark:text-orange-400">{p.value}</span>
                      </div>
                    ))}
                  </div>
                </>
            ...
            <div className="lg:col-span-4 bg-white dark:bg-gray-800 rounded-xl shadow-lg border border-gray-100 dark:border-gray-700 p-6 flex flex-col h-full">
              <div className="flex items-center justify-between mb-6">
                <h3 className="text-lg font-bold text-gray-900 dark:text-white">Out of Stock</h3>
              ...
              <div className="space-y-3 flex-1">
                {displayedStock.length === 0 ? (
                  <div className="text-center py-8">
                    ...
                    <p className="text-green-600 dark:text-green-400 font-medium">All stock levels healthy!</p>
                  </div>
                ) : (
                  <>
                    {displayedStock.map((p, idx) => (
                      <div key={p.id + '-' + idx} className="flex items-center justify-between p-3 bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-700 rounded-lg">
                        <div className="flex items-center space-x-3">
                          <div className="w-2 h-2 bg-red-500 rounded-full"></div>
                          <span className="text-sm font-medium text-red-800 dark:text-red-200">{p.name}</span>
                        </div>
                        <span className="text-xs font-bold text-red-600 dark:text-red-400 bg-red-100 dark:bg-red-900/30 px-2 py-1 rounded">OUT OF STOCK</span>
                      </div>
                    ))}
                    {outOfStockItems.length > STOCK_DISPLAY_LIMIT && (
                      <button
                        className="text-orange-600 dark:text-orange-400 underline text-sm mt-2"
            ...
            <div className="lg:col-span-4 bg-white dark:bg-gray-800 rounded-xl shadow-lg border border-gray-100 dark:border-gray-700 p-6 flex flex-col h-full">
              <div className="flex items-center justify-between mb-6">
                ...
                <span className="text-xs bg-orange-100 dark:bg-orange-900/20 text-orange-600 dark:text-orange-400 px-2 py-1 rounded-full font-medium">Last 7 days</span>
              </div>
              <div className="space-y-3 flex-1">
                    ...
                    {displayedRecentSales.map(sale => (
                      <div key={sale.id} className="flex items-center justify-between p-3 hover:bg-gray-50 dark:hover:bg-gray-700 rounded-lg transition-colors">
                        ...
                        <div className="text-right">
                          <div className="text-sm font-bold text-orange-600 dark:text-orange-400">${sale.total}</div>
        </div>
          </div>
                    ))}
          ...
          <div className="bg-white dark:bg-gray-800 rounded-xl shadow-lg border border-gray-100 dark:border-gray-700 p-6 mb-8">
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
              <div>
                ...
                <p className="text-sm text-gray-600 dark:text-gray-400 mt-1">Track your revenue performance over time</p>
        </div>
                ...
                <div className="flex items-center space-x-2">
                  {trendView === 'monthly' && (
                    <select 
                      ...
                      className="border border-gray-300 dark:border-gray-600 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-orange-500 focus:border-orange-500 bg-white dark:bg-gray-700 text-gray-900 dark:text-white"
                    >
                {[...Array(12)].map((_, i) => (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            <div className="bg-white dark:bg-gray-800 rounded-xl shadow-lg border border-gray-100 dark:border-gray-700 p-6 text-center">
              ...
              <div className="text-xs text-green-600 dark:text-green-400 mt-2">+{growthPercentage}% vs last month</div>
            </div>
            ...
            <div className="bg-white dark:bg-gray-800 rounded-xl shadow-lg border border-gray-100 dark:border-gray-700 p-6 text-center">
              ...
              <div className="text-2xl font-bold text-gray-900 dark:text-white mb-1">{lowStock.length}</div>
              <div className="text-sm text-gray-600 dark:text-gray-400">Products in Stock</div>
              <div className="text-xs text-blue-600 dark:text-blue-400 mt-2">{lowStockCritical.length} need attention</div>
            </div>
            ...
            <div className="bg-white dark:bg-gray-800 rounded-xl shadow-lg border border-gray-100 dark:border-gray-700 p-6 text-center">
              ...
              <div className="text-2xl font-bold text-gray-900 dark:text-white mb-1">{last7DaysSales.length}</div>
              <div className="text-sm text-gray-600 dark:text-gray-400">Sales This Week</div>
              <div className="text-xs text-green-600 dark:text-green-400 mt-2">+15% vs last week</div>
            </div>
            ...
            <div className="bg-white dark:bg-gray-800 rounded-xl shadow-lg border border-gray-100 dark:border-gray-700 p-6 text-center">
              ...
              <div className="text-sm text-gray-600 dark:text-gray-400">Top Product</div>
              <div className="text-xs text-purple-600 dark:text-purple-400 mt-2">{bestsellers[0]?.total_sold || 0} sold</div>
            </div>
          </div>
        </div>
      </div>
      ...
      {showUpdateModal && selectedProduct && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50 p-4">
          <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-2xl w-full max-w-md transform transition-all">
            <div className="p-6">
              <div className="flex items-center justify-between mb-6">
                <h2 className="text-2xl font-bold text-gray-900 dark:text-white">Update Product</h2>
                <button 
              ...
              <form onSubmit={handleUpdateProduct} className="space-y-4">
                <div>
                  ...
                  <input 
                    type="text" 
                    className="w-full border border-gray-300 dark:border-gray-600 rounded-lg px-4 py-3 focus:outline-none focus:ring-2 focus:ring-orange-500 focus:border-orange-500 bg-white dark:bg-gray-700 text-gray-900 dark:text-white" 
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Part Number</label>
                    <input 
                      type="text" 
                      className="w-full border border-gray-300 dark:border-gray-600 rounded-lg px-4 py-3 focus:outline-none focus:ring-2 focus:ring-orange-500 focus:border-orange-500 bg-white dark:bg-gray-700 text-gray-900 dark:text-white" 
